                                       ...
                                       const Eigen::Matrix4d &M, const Eigen::Matrix<double, 6, 1> &aff_screw)
{
    ...
    // Alternative method using TF data
    /* tf2_ros::Buffer tf_buffer; */
    /* const Eigen::Isometry3d ee_htm = */
    /*     AffordanceUtilROS::get_htm("arm0_base_link", "arm0_tool0", tf_buffer);
     */
    /* const Eigen::Vector3d q_vir = ee_htm.translation(); */
...
Eigen::Matrix<double, 6, 1> get_aff_screw_from_tag(const Eigen::Vector3d &w_aff, const std::string &reference_frame,
...
{
  public:
    CcAffordancePlannerRosNode(const std::string &robot_config_file_path, const Eigen::VectorXd &aff_screw,
                               const double &aff_goal)
        : nh_("~"),
          aff_screw_(aff_screw),
          aff_goal_(aff_goal),
          client_("/spot_arm/arm_controller/follow_joint_trajectory", true)
    {

        // Subscribers
        joint_states_sub_ = nh_.subscribe("/joint_states", 1000, &CcAffordancePlannerRosNode::joint_states_cb_, this);
        ...
        const size_t real_robot_joints = robotConfig.joint_names.size() - 3; // Disregard virtual joint names
        joint_names_.assign(robotConfig.joint_names.begin(), robotConfig.joint_names.begin() + real_robot_joints);
    ...
    void run_cc_affordance_planner()
    {
        ...
        Eigen::MatrixXd robot_slist = h_slist_.leftCols(joint_states_.positions.size());
        // Compose cc model and affordance goal
        Eigen::MatrixXd cc_slist = compose_cc_model_slist(robot_slist, joint_states_.positions, M_, aff_screw_);
        ...
        /* ccAffordancePlanner.affStep = sign_of(aff_goal_) * 0.2; // should have the same sign as aff_goal_ */
        ...
        const double aff_step = 0.01;
        const double accuracy = 10.0 / 100.0;
        ccAffordancePlanner.p_aff_step_deltatheta_a = sign_of(aff_goal_) * aff_step;
        ccAffordancePlanner.p_task_err_threshold_eps_s = accuracy * aff_step;
        const int task_offset = 1;

        // Run the planner
        PlannerResult plannerResult = ccAffordancePlanner.affordance_stepper(cc_slist, aff_goal_, task_offset);
        ...
        {
            std::cout << "Planner succeeded with "
                      << plannerResult.traj_full_or_partial
        ...
        ros::ServiceClient moveit_plan_and_viz_client =
            nh_.serviceClient<moveit_plan_and_viz::MoveItPlanAndViz>(plan_and_viz_service_name);

        moveit_plan_and_viz::MoveItPlanAndViz moveit_plan_and_viz_goal;
        moveit_plan_and_viz_goal.request.joint_traj = goal.trajectory;

        ROS_INFO_STREAM("Waiting for " << plan_and_viz_service_name << " service to be available");
        ros::Duration timeout(60.0);
        if (!ros::service::waitForService(plan_and_viz_service_name))
        {
            ROS_INFO_STREAM(plan_and_viz_service_name << " service isn't available. Timed out waiting");
        }
        else
        {
            ROS_INFO_STREAM(plan_and_viz_service_name << " service found. Now sending goal.");
        }

        if (moveit_plan_and_viz_client.call(moveit_plan_and_viz_goal))
        {
            ROS_INFO_STREAM("Calling " << plan_and_viz_service_name << " service was successful.");
        ...
        {
            ROS_INFO_STREAM("Failed to call " << plan_and_viz_service_name << " service.");
        }
        // Execute plan
    }
...
int main(int argc, char **argv)
{
    ...
    if (aff_from_tag)
    {
        ...
        const std::string reference_frame = "arm0_base_link";    // Maybe we can move the aff_from_tag portion inside
                                                                 ...
                                                                 // available there.
        aff_screw = get_aff_screw_from_tag(w_aff, reference_frame, affordance_frame, tf_buffer);
    }
    else
    {
    ...
    CcAffordancePlannerRosNode ccAffordancePlannerRosNode(robot_cc_description_path, aff_screw, aff_goal);
